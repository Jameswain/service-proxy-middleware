...
module.exports = {
	...
	runProxyRule({ context, enable, target }, filename, opts = {}) {
		const proxyOptions = {
			...
			onProxyRes: function onProxyReq(proxyRes, req, res) {
				...
				proxyRes.headers['service-proxy-middleware-5-match'] = target + req.url;
			},
			...opts,
		};
	...
	watchProxyRuleFile(options) {
		...
		watcher.on('all',  (event, path) => {
			...
			options.realtimeLog && this.printProxyRule(options.mapProxyRules);
			// 刷新浏览器
			if (server) {
				if(typeof server.sockWrite === 'function') { // webpack-dev-server 3
					server.sockWrite(server.sockets, 'content-changed');
				}
				if(typeof server.sendMessage === 'function' && server.sockets && server.sockets.length) { // webpack-dev-server 4
