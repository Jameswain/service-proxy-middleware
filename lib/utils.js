...
module.exports = {
	...
	runProxyRule({ context, enable, target }, filename, opts = {}) {
		const proxyOptions = {
			...
			onProxyRes: function onProxyReq(proxyRes, req, res) {
				...
				proxyRes.headers['service-proxy-middleware-match'] = context;
				proxyRes.headers['service-proxy-middleware-target'] = path.join(target, req.url);
				proxyRes.headers['service-proxy-middleware-pathname'] = url.parse(req.url).pathname;
			},
			...opts,
		};
	 ...
	 */
	getEntryWithProxyMap({ webpackConfig, filename, publicPath }) {
		const mapHtml = webpackConfig.plugins.reduce((previous, plugin, index, array) => {
			if (plugin instanceof HtmlWebpackPlugin) {
				previous[path.join(publicPath, plugin.options.filename)] = new Set();
